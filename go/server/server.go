...
func handleAddGeohashRequest(msg *messages.AddGeohashRequest, mh *messages.MessageHandler, cnode *chord.Node, gr *geode.GeoRange) {
	defer mh.Close()
	for _, geohash := range msg.GetGeohashes() {
		...
		if err != nil {
			log.Printf("error finding target for: %s, %s", geohash, err)
			continue
		}
		err = messages.SendAddHashRequest(tgtNode.GetAddr(), []string{geohash})
		if err != nil {
