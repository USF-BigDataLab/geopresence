...
static void query_transform(roaring_bitmap_t *r, struct geode *g,
                            const struct spatial_range *coords, int n);
...
struct geode *geode_create(const char *base_geohash, unsigned int precision,
  ...
  LOG("New GEODE: %s (%d x %d), dpp: (%f, %f)\n", g->prefix, g->width,
      g->height, g->x_px, g->y_px);

  return g;
}

void geode_free(struct geode *g) {
  roaring_bitmap_free(g->bmp);
  HLL_free(g->hll);
...
/**
 * Add a data point to a geode. Search for and create sub-geodes when geode is
 * appropriately dense.
 *
...
void geode_add_geohash(struct geode *g, const char *geohash) {
  ...
  if (lf > .6) {
    ...
    /* If sub geode wasn't found and it will be at most 10 characters in
     * length, create it */
    if (g->prefix_sz < 10) {
      g->sgs[g->num_sgs] = geode_create(geohash, 16, g->prefix_sz + 1);
      LOG("Created sub geode -> %s -> sz %d\n", g->sgs[g->num_sgs]->prefix,
          g->sgs[g->num_sgs]->prefix_sz);
      gx = g->sgs[g->num_sgs];
      g->num_sgs++;
    }
  }
...
void print_geocoord(GeoCoord *gc) {
  printf("(%f, %f) in [%f, %f, %f, %f]\n", gc->longitude, gc->latitude,
         gc->north, gc->east, gc->south, gc->west);
}
...
void print_geode(struct geode *gc) {
  printf("%s (%f, %f) in [%f, %f, %f, %f]\n", gc->prefix,
         gc->base_range.longitude, gc->base_range.latitude,
         gc->base_range.north, gc->base_range.east, gc->base_range.south,
         gc->base_range.west);
}
 ...
 * Purpose:  check ifany index locations that are set in g are contained in the
 * bounds of coords Params:   g      - the geode to query coords - the polygon
 * represented as lat/lon pairs n      - number of lat/lon pairs
 */
bool polygon_intersects_geode(struct geode *g,
                              const struct spatial_range *coords, int n) {
  roaring_bitmap_t *r =
      roaring_bitmap_create_with_capacity(g->width * g->height);
 ...
 * Purpose:  get the index locations that are set in g and contained in the
 * bounds of coords Params:   g      - the geode to query coords - the polygon
 * represented as lat/lon pairs n      - number of lat/lon pairs
 *
 * Return:   locations_and_sz - array of matching locations and the size of the
 * array. the size is determined with roaring_bitmap_get_cardinality
 */
struct query_result *polygon_query_geode(struct geode *g,
                                         const struct spatial_range *coords,
                                         int n) {
  roaring_bitmap_t *r =
      roaring_bitmap_create_with_capacity(g->width * g->height);
  query_transform(r, g, coords, n);

  /* Convert the intersecting grid cells of the geode and the query to an array
   * of matching index locations */
 ...
 *           g - The geode that the bitmap should be drawn for. If any part of
 * the polygon is outside this geode's spatial range, that part of the polygon
 * will be clipped c - Set of latitude and longitude coordinates n - number of
 * coordinates
 */
void query_transform(roaring_bitmap_t *r, struct geode *g,
